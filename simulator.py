...
def main(arglist):
    ...
    try:
        agent = RLAgent(env)
        # env2 = GameEnv(input_file)
        # agent2 = RLAgent(env2)
        # env3 = GameEnv(input_file)
        # agent3 = RLAgent(env3, learning_rate=0.5)
        if not WINDOWS and not DEBUG_MODE:
            signal.alarm(0)
    ...
    try:
        ...
        # plt.plot(agent2.rolling_averages, 'b')
        # plt.plot(agent.rolling_averages, 'tab:orange')
        # plt.plot(agent3.rolling_averages, 'g')

        # plt.xlabel("Episode #")
        # plt.ylabel("50-Step Moving Average Reward")
        # plt.title("Policy Quality vs Episode #")
